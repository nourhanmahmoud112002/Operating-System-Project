...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	for(int i=(uint32)virtual_address; i<(uint32)virtual_address+shares[index].size; i+=PAGE_SIZE)
	{
		...
		{
			map_frame((myenv->env_page_directory),frame,(uint32)virtual_address,PERM_WRITEABLE|PERM_USER|PERM_PRESENT );
		}
		else
			map_frame((myenv->env_page_directory),frame,(uint32)virtual_address,PERM_USER|PERM_PRESENT);
